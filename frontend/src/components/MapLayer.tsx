...
export const MapLayer = ({layerConfig, pane}: MapLayerProps) => {
    ...
    const {data, isLoading, dataUpdatedAt} = useLayerData(layerConfig, bbox, isVisible);
        ...
        layer.on({
                ...
                if (geomType === 'Polygon' || geomType === 'MultiPolygon') {
                    ...
                    if (targetLayer.setStyle) {
                        targetLayer.setStyle({
                            weight: (targetLayer.options.weight || 3) + 1,
                            opacity: (targetLayer.options.opacity || 1) - 0.1
                        });
                    }
                }
            },
            ...
            mouseout: (e) => {
                ...
                if (geomType === 'Polygon' || geomType === 'MultiPolygon') {
                    ...
                    if (targetLayer.setStyle) {
                        targetLayer.setStyle({
                            weight: (targetLayer.options.weight || 4) - 1,
                            opacity: (targetLayer.options.opacity || 1) + 0.1
                        });
                    }
                }
    ...
    return (
        <GeoJSON
            key={`${layerConfig.id}-${dataUpdatedAt}`}
            pane={pane}
            data={data}
